...
latex_table = '\n'.join(lines)
with open(os.path.join(out_dir, "tables/severe_shocks_GDP_closed.tex"), "w") as f:
    f.write(latex_table)
...
plt.tight_layout()
plt.savefig(os.path.join(out_dir, 'figures/aggregate_responses_with_cd_closed.pdf'), dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(out_dir, 'figures/aggregate_responses_closed.pdf'), dpi=300, bbox_inches='tight')
plt.close()
...
stats_table.to_latex(os.path.join(out_dir, 'tables/calibrated_shocks_GDP_closed.tex'), index=False, escape=False)
...
top_3_price_table.to_latex(os.path.join(out_dir, 'tables/calibrated_shocks_top3_closed.tex'), index=False, escape=False)
